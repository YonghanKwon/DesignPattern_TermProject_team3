...
public final class Database { /*
	...
	/**
	 * The number of rows modified by the last INSERT, DELETE, or UPDATE request.
	 */
	private int affectedRows = 0;

	/**
	 * This Map holds the tables that are currently active. I have to use be a Map
	 * (as compared to a Set), because HashSet uses the equals() function to resolve
	 * ambiguity. This requirement would force me to define "equals" on a Table as
	 * "having the same name as another table," which I believe is semantically
	 * incorrect. Equals should match both name and contents. I avoid the problem
	 * entirely by using an external key, even if that key is also an accessible
	...
	private final class TableMap implements Map {
		...
		/**
		 * If the requested table is already in memory, return it. Otherwise load it
		 * from the disk.
		 */
	...
	private static final Token
		...
		// --------------------------------------------------------------
		/**
		 * Create a database object attached to the current directory. You can specify a
		 * different directory after the object is created by calling
		 * {@link #useDatabase}.
		 */
		public Database() {
		}
		...
		/**
		 * Use this constructor to wrap one or more Table objects so that you can access
		 * them using SQL. You may add tables to this database using SQL "CREATE TABLE"
		 * statements, and you may safely extract a snapshot of a table that you create
		 * in this way using:
		 * 
		 * <PRE>
		...
		/**
		 * Asks the scanner to throw a {@link ParseFailure} object that highlights the
		...
		/**
		 * Use an existing "database." In the current implementation, a "database" is a
		 * directory and tables are files within the directory. An active database
		 * (opened by a constructor, a USE DATABASE directive, or a prior call to the
		 * current method) is closed and committed before the new database is opened.
		 * 
		 * @param path A {@link File} object that specifies directory that represents
		 *             the database.
		 * @throws IOException if the directory that represents the database can't be
		 *                     found.
		 */
		 ...
		 * files in that directory. If you don't call this method (or issue a SQL CREATE
		 * DATABASE directive), then the current directory is used.
		 * 
		...
		/**
		 * Destroy both internal and external (on the disk) versions of the specified
		 * table.
		 */
		...
		/**
		 * Flush to the persistent store (e.g. disk) all tables that are "dirty" (which
		 * have been modified since the database was last committed). These tables will
		 * not be flushed again unless they are modified after the current dump() call.
		 * Nothing happens if no tables are dirty.
		 * <p>
		 * The present implemenation flushes to a .csv file whose name is the table name
		 ...
		 * {@link #execute} call. Zero is returned for all operations except for INSERT,
		 * DELETE, or UPDATE.
		 */
		...
		// @parser-start
		/*******************************************************************
		 * Execute a SQL statement. If an exception is tossed and we are in the middle
		 * of a transaction (a begin has been issued but no matching commit has been
		 * seen), the transaction is rolled back.
		 *
		 ...
		 *                   statement must be present (you cannot break a long
		 *                   statement into multiple calls), and text following the SQL
		 ...
		 *         requests. The result table is treated like a normal database table if
		 *         the SELECT contains an INTO clause, otherwise it's a temporary table
		 *         that's not put into the database.
		 *
		...
		private interface Expression { /*
										 * Evaluate an expression using rows identified by the two iterators passed as
										 * arguments. <code>j</code> is null unless a join is being processed.
										 */
		...
		private class ArithmeticExpression implements Expression {
			...
			public Value evaluate(Cursor[] tables) throws ParseFailure {
				...
				verify(leftValue instanceof NumericValue && rightValue instanceof NumericValue,
		...
		private class LikeExpression implements Expression {
			...
			public Value evaluate(Cursor[] tables) throws ParseFailure {
				Value leftValue = left.evaluate(tables);
				Value rightValue = right.evaluate(tables);
				verify(leftValue instanceof StringValue && rightValue instanceof StringValue,
		...
		private final class IdValue implements Value {
			...
			/**
			 * Using the cursor, extract the referenced cell from the current Row and return
			...
			/**
			 * Using the cursor, extract the referenced cell from the current row of the
			 * appropriate table, convert the contents to a {@link NullValue},
			 * {@link NumericValue}, or {@link StringValue}, as appropriate, and return that
			 * value object.
			 */
		...
		// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		private int doUpdate(String tableName, final String columnName, final Expression value, final Expression where)
				throws ParseFailure {
			Table t = (Table) tables.get(tableName);
			try {
